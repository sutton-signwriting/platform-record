...
> The platform separates technical preservation from orthographic prescription, and treats literacy outcomes as a research horizon rather than a completed claim.
...
Low visibility in one research tradition is not evidence of absence. At the same time, documented use is not evidence of global scale. The evidence layer should identify what is public, where it is anchored, which communities or institutions are involved, what kind of use is documented, and where the record remains bounded.
...
Orthographic norms should emerge through community use, teaching practice, lexicography, publication, correction, and corpus evidence. Corpus frequency does not automatically decide correctness, because frequency can reflect one school, one dictionary, one tool, one region, or one institutional center. But stable text infrastructure makes community usage observable. It allows researchers and communities to ask which forms are used, by whom, where, in what context, and with what degree of stability.
...
For spatial signed writing, a character repertoire does not by itself define sign boundaries, signbox composition, coordinate structure, sorting behavior, search behavior, rendering rules, migration paths, or equivalence policies. A font can draw glyphs; it does not decide what a written sign is. A renderer can make text visible; it does not author the sign or settle orthographic identity.

The platform separates Sutton SignWriting as the human writing system, Formal SignWriting as a technical text model, FSW and SWU as production encodings of that model, fonts and SVG as rendering layers, and official Unicode SignWriting as a standards-facing character layer with its own history and compatibility questions.
...
The relevant question is whether the system supports conventional written units that can be authored, read, corrected, cited, taught, searched, archived, and reused as text. SignWriting should be compared with other writing systems by asking both what it represents and how its primary written unit is organized.

The platform's bounded claim is:

> SignWriting is not merely a picture, symbol inventory, font, or specialist transcription. It is a human writing system whose primary written unit is a composed signbox.
...
The existence of many possible symbols is not the same as the number of symbols a learner meets in a curriculum, a dictionary, a classroom, or a local writing practice. Spatial composition creates a learning task, but it also preserves written relations that a linear list would lose. Digital tools can reduce or increase difficulty depending on how they handle symbol selection, placement, correction, search, and reuse.
...
Its purpose is to prevent a complex body of writing-system, technical, evidence, standards, and stewardship work from remaining scattered, uncitable, or dependent on informal web pages. Once claims are stable, other people can cite them, reject them, test them, audit them, extend them, or correct them.
...
| who can authorize use in schools or communities | governance and deployment | technical stewardship |
| self-archiving, internal citation, or authorial interest | scholarly posture | evidence that every technical claim is false |
| video, AI, annotation, or corpora | adjacent documentation and computational layers | durable written signed-language text |
| official Unicode status | standards and character encoding | complete production text infrastructure |
| glyph appearance or font behavior | rendering | human written-unit identity |
| iconicity or visual motivation | grapholinguistic typology | proof that the system is not writing |
| symbol inventory size or tool difficulty | pedagogy and usability | the existence of a written text model |
| DOI archiving and platform structure | citation infrastructure | scholarly consensus |
...
- a text model is not a literacy outcome
- Unicode status is not a complete production ecosystem
- video documentation preserves signed performance; it is not itself written text
- author-responsible archiving is not independent consensus
